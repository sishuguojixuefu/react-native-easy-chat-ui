...
import { TouchableWithoutFeedback, View, TouchableOpacity, Text, Image, StyleSheet, Dimensions } from "react-native";
...
export default class ChatItem extends PureComponent {
  ...
  _matchContentString = (textContent, views, isSelf) => {
    ...
    if (emojiIndex === -1) {
      views.push(
        <Text style={isSelf ? rightMessageTextStyle : leftMessageTextStyle} key={"emptyTextView" + Math.random() * 100}>
          {textContent}
        </Text>
      );
    } else {
      ...
      // 将0-index部分返回文本
      views.push(
        <Text style={isSelf ? rightMessageTextStyle : leftMessageTextStyle} key={"firstTextView" + Math.random() * 100}>
  ...
  renderCheck = () => {
    if (this.props.renderMessageCheck === undefined) {
      if (this.state.isSelect) {
        return <View style={styles.check}>{this.props.messageSelectIcon}</View>;
      } else {
        ...
        >
          <View>
            ...
            >
              ...
              {isSelf && isOpen && type !== "system" && <View style={{ flex: 1 }}>{this.renderCheck()}</View>}
            </TouchableOpacity>
